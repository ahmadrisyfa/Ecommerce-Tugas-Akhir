...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ url('admin/users/'.$users->id.'/edit') }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            <label for="title">Name</label>
                            <input type="text" class="form-control" value="{{ old('name', $users->name )}}" name="name">
                            @error('name')
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="email" readonly class="form-control" value="{{ old('email', $users->email )}}"  name="email">
                            @error('email')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>   
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <select name="role_as" id="role_as" class="form-select">
                                ...
                                <option value="0" @if (old('role_as',$users->role_as) == "0") {{ 'selected' }} @endif>s</option>
                                <option value="1" @if (old('role_as',$users->role_as) == "1") {{ 'selected' }} @endif>Admin</option>
                            </select>
                            @error('status')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
